...
func selectMF(card Card) error {
	// SELECT FILE by FID 3F00, no FCI.
	apdu := []byte{claISO, insSelect, p1SelectMF, p2NoFCI, 0x02, byte(fidMF >> 8), byte(fidMF & 0xFF)}
	...
	if sw != sw9000 {
		_, sw, err = transmit(card, []byte{claISO, insSelect, p1SelectMF, p2NoFCI})
		if err != nil {
			return err
		}
		if sw != sw9000 {
...
func selectByAID(card Card, aid []byte) ([]byte, error) {
	...
	apdu := append([]byte{claISO, insSelect, p1SelectAID, p2RetFCP, byte(len(aid))}, aid...)
	apdu = append(apdu, 0x00) // Le=0 → return up to 256 bytes of FCP
	data, sw, err := transmit(card, apdu)
	if err != nil {
		return nil, err
	}
	if sw&0xFF00 == sw61xxHighByte {
		// Need GET RESPONSE — older cards
		more, _, gerr := transmit(card, []byte{claISO, insGetResp, 0x00, 0x00, byte(sw & 0xFF)})
		if gerr == nil {
	...
	if sw != sw9000 {
		...
		apdu = append([]byte{claISO, insSelect, p1SelectAID, p2NoFCI, byte(len(aid))}, aid...)
		_, sw, err = transmit(card, apdu)
		if err != nil {
			return nil, err
		}
...
func selectEF(card Card, fid uint16) error {
	apdu := []byte{claISO, insSelect, p1SelectEF, p2NoFCI, 0x02, byte(fid >> 8), byte(fid & 0xFF)}
	_, sw, err := transmit(card, apdu)
	if err != nil {
		return err
	}
...
func readBinary(card Card, sfi byte, offset, length uint16) ([]byte, error) {
	var p1, p2 byte
	if sfi != 0 {
		if offset > readBinaryByteOffsetMax {
			return nil, fmt.Errorf("SFI offset %d > 255", offset)
		}
		p1 = readBinarySFIBit | (sfi & readBinarySFIMask)
		p2 = byte(offset)
	} else {
		p1 = byte(offset >> 8)
		p2 = byte(offset)
	}
	apdu := []byte{claISO, insReadBinary, p1, p2, byte(length & 0xFF)}
	data, sw, err := transmit(card, apdu)
	if err != nil {
		return nil, err
	}
	...
	if sw != sw9000 && sw != sw6282EndOfFile {
...
func readEFBySFI(card Card, sfi byte, fid uint16) ([]byte, error) {
	...
	for uint16(len(out)) < total {
		...
		var buf []byte
		if off <= readBinaryByteOffsetMax {
